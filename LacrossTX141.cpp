...
LacrossTX141::LacrossTX141() : Device() {
   syncCount = 0;
   syncFound = false;
   bitCount = 0;
   code = 0;
...
void LacrossTX141::processPulse(long duration) {
...
void LacrossTX141::publishTopic(Message* message, char* buffer, int maxLength) {
   char* bytes = (char*) &message->code;
   snprintf(buffer, maxLength, "house/lacrossTX141/%x/temp", bytes[3]);
}
...
bool LacrossTX141::validateChecksum(int code) {
