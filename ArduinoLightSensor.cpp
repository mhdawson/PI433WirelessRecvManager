...
ArduinoLightSensor::ArduinoLightSensor() : Device() {
   syncCount = 0;
   syncFound = false;
   bitCount = 0;
   code = 0;
...
void ArduinoLightSensor::processPulse(long duration) {
...
void ArduinoLightSensor::publishTopic(Message* message, char* buffer, int maxLength) {
...
void ArduinoLightSensor::publishTopic(int messageNum, Message* message, char* buffer, int maxLength) {
   char* bytes = (char*) &message->code;
   snprintf(buffer, maxLength, "house/arduinoLightSensor/%x", bytes[3]);
}
...
bool ArduinoLightSensor::validateChecksum(int code) {
